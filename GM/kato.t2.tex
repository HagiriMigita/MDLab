...
\begin{document}
	...
	\begin{itemize}
		...
		\item v2では解像度・視野の補正、アノテーションの変換を行っているので、v1から画像の枚数は減ってしまったが、信頼性の高いデータが含まれている。結果として、v2はscoreのグラフが右に移動している。(図4) 
		\item また、v1+v2ではデータ数が増加しているので、全体的にv1よりscoreのグラフが右に移動している。(図4)
	...
	\begin{figure}[H]
		...
		\begin{minipage}[H]{0.3\columnwidth}
			...
			\caption*{v1+v2}
		\end{minipage}
		\caption{モデルによる各アノテーションへの確信度の分布}
	\end{figure}
	\begin{table}[h]
		\centering
	...
	\section{前回のLTからの進捗}
		...
		\begin{itemize}
			...
			\item 実際にRGBとThermalのannotationsファイルのbbox情報を画像の上に重ねて出力したところ、わずかなズレはあるものの、おおむね一致している。
		\end{itemize}
		\begin{figure}[H]
			\centering
			\includegraphics[height=5cm]{fig/RGB_Thermal_GT.jpg}
			\caption{v2のRGBとThermalのGTのbboxの比較}
		\end{figure}
		\subsection{疑似ラベルにおけるscoreについて}
		\begin{itemize}
			\item 疑似ラベルのscoreについて確認したところ、事前学習モデルにより画像から検出された物体に対する確信度合いであり、IoUスコアではないことがわかった。
			\item また疑似ラベルでは事前学習モデルによって検出された物体は、GTが想定している物体と異なるものも存在する。例えばキックボードに乗っている人をpersonとして認識する場合があった。
			\item そのため、疑似ラベルのscoreはGTのbboxと比較したIoUスコアとは異なることに注意する必要がある。
		\end{itemize}
	...
	\section{今後の課題\&スケジュール}
		\begin{itemize}
			\item 現在、v1とv2は全てのデータセットにおいてのテストデータを出している。v1+v2用損失関数のチューニングが終了次第、結果の比較を行う。
		\end{itemize}
