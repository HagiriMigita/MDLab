...
- 損失関数に関する比較としては、v1同士の結果比較で、新損失関数のTune後


- ハイパーパラメーターチューニング後、CarのmAP_mのみ低下したが、それ以外の指標は大幅に上昇した。 
