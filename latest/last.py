...
if __name__ == "__main__":
    ...
    surprise_image = cv2.imread('surprise.png', cv2.IMREAD_UNCHANGED)
    ...
    if surprise_image is None:
        print("Error: surprise.png could not be loaded. Please check the file path.")
        cap.release()
        cv2.destroyAllWindows()
        exit()
    ...
    surprise_image = cv2.cvtColor(surprise_image, cv2.COLOR_BGRA2RGBA)
    ...
    with torch.no_grad():
        ...
        while cap.isOpened():
            ...
            if 'anger' in label:
                ...
                # Resize the anger_frame to match the size of the frame
                resized_anger_frame = cv2.resize(anger_frame, (frame.shape[1], frame.shape[0]))
                
                frame_flags = True
                paste_image(frame, resized_anger_frame, 0, 0)
            ...
            if 'surprise' in label:
                frame_flags = True
                resized_surprise_image = cv2.resize(surprise_image, (frame.shape[1], frame.shape[0]))
                paste_image(frame, resized_surprise_image, 0, 0)
